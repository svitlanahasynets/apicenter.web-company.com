...
class Features extends MY_Controller {
	...
	public function index(){
		$variables = array();
        $variables['page_title'] = translate('Features')
        ;
		$variables['go_back_url'] = site_url('/features');
		...
		$data['views'] = array('integrations/features/index');
		$data['js'] = array(
	...
	public function edit(){
		...
		$data['helpers'] 	= array('form');
		$data['libraries'] 	= array('Pmprojects');
		$data['views'] 		= array('integrations/features/edit');
	...
	public function getResponse($display = true, $isExport = false){
		...
		if(!empty($projects)){
			if($display){
				...
				$html = $this->load->view(TEMPLATE.'/integrations/features/results', $data, true);
				$return = array(
					'html' => $html,
					'pages' => $numberOfPages
				);
