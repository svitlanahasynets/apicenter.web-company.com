...
class Schedule extends MY_Controller {
	...
	public function index(){
		$variables = array();
        $variables['page_title'] = translate('Schedule')
        ;
		$variables['go_back_url'] = site_url('/schedule');
		...
		$data['views'] = array('integrations/schedule/index');
		$data['js'] = array(
	...
	public function edit(){
		...
		$data['helpers'] 	= array('form');
		$data['libraries'] 	= array('Pmprojects');
		$data['views'] 		= array('integrations/schedule/edit');
	...
	public function getResponse($display = true, $isExport = false){
		...
		if(!empty($projects)){
			if($display){
				...
				$html = $this->load->view(TEMPLATE.'/integrations/schedule/results', $data, true);
				$return = array(
					'html' => $html,
					'pages' => $numberOfPages
				);
