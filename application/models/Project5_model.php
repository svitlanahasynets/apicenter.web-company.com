...
class Project5_model extends CI_Model {
	...
	public function checkConfigurable($saveData, $productData, $projectId, $type = ''){
		...
		if($productData['is_configurable'] == true && $productData['is_configurable'] != 'false'){
			...
			log_message('debug', 'Starfood Configurable: '. var_export($childProducts, true));
...
", '|br|', $resultData);
	...
	public function getArticleData($articleData, $finalArticleData){
	    ...
	    // Levertijd
		if(isset($articleData['Levertijd']) && $articleData['Levertijd'] != '' && $articleData['Levertijd'] != null)
		{
			$finalData['custom_attributes']['levertijd_afas'] = array(
			    'type' => 'dropdown',
