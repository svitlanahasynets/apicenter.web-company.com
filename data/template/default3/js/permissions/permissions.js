	...
	$("#permissions-table").fancytree({
		...
		renderColumns: function(event, data) {
			...
			if(node.data.type=='project'){
				...
				details = "<a href='"+$("input#site-url").val()+"/permissions/AccrediterenField/id/"+$("input#user_id").val()+"/pid/"+node.data.id+"'> Accrediteren</a>";

				partner_section = '<select name="partner_id[' + node.data.id + ']" class="form-control m-input m-input--air">';
				partner_section += '<option value=""> -- Select Partner -- </option>'

				var partner_ids_str = node.data.partner_ids_str;
				var partner_ids_array = partner_ids_str.split(",");

				for (var i = 0; i < partner_ids_array.length; i++) {
					var partner_id_array = partner_ids_array[i].split("-");
					var partner_id = partner_id_array[0];
					var partner_name = partner_id_array[1];

					partner_section += '<option value="' + partner_id + '">' + partner_name + '</option>';
				}

				partner_section += '</select>';

			}
			
			$tdList.eq(4).html(details);
			$tdList.eq(5).html(partner_section);
		}
	});
